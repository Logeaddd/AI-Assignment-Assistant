...
如果课件是扫描版、图片公式或 Office 公式，当前版本可能识别不到公式。

当系统显示“自动抽取公式数为 0”时，公式/推导题不能当作课件标准答案，只能作为风险草稿。

这时必须用页图、OCR、人工或支持图片输入的模型进一步核对。

开发者/agent 可以使用 `extract-formula-pages` 生成公式页核对包，再交给支持图片输入的模型提取公式。
