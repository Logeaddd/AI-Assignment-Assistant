...
面向授课教师(和像我一样爱偷懒的学生)的**“课件口径标准答案”本地 harness**。它把 PDF / Word / PPT 课件解析成课程知识库，按题目检索课件证据，再生成或辅助生成标准答案——并且在生成前先**诚实地判断课件到底能不能支撑这道题**。
...
直接使用ai来解答的话,ai会偷懒 会以自己的方式生成,这样很容易被发现用了ai!
...
- **公式识别失败时不许冒充标准答案。** 扫描/图片型课件若自动抽取公式数为 0，公式/推导题只能输出风险草稿，不能标为课件标准答案。
